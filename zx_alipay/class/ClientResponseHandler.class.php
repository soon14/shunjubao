...
class ClientResponseHandler  {
	...
	*/	
	function isTenpaySign() {
		...
		$dtime = time();
		$sign = md5($tmch_id.$keyw.$dtime);
		$url="http://quan.zhiying365.com/get_payment_one.php?tmch_id=$tmch_id&dtime=$dtime&sign=$sign";
		$get_key = file_get_contents($url);
		$new_key = json_decode($get_key,true); 
		$mkey = $new_key["mkey"];
		

		$this->log_result_sign("log_result_sign.txt",$tmch_id."_".$mkey);
		if(!empty($mkey)){
			$signPars .= "key=".$mkey;	
		}else{
	...
	*/	
	...
	function _isTenpaySign($signParameterArray) {
	 ...
	 function  log_result_sign($file,$word){
	    $fp = fopen($file,"a");
	    flock($fp, LOCK_EX) ;
