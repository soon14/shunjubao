...
class CorefireAliPayNotify extends CorefireAliPayNotifyReply
{
	/**
	 * 
	 * 回调入口
	...
	private function notify($callback, &$msg)
	{
		...
		//如果返回成功则验证签名
		try {
			$obj = new CorefireAliPayDataBase('');
			$obj->FromXml($xml);
			...
			}else 
			{
				...
				$this->SetKey($key);
				//$result = CorefireAliPayResults::Init($xml,$key);
				$result = $obj->GetValues();
			}

			return call_user_func($callback, $result);
			
		} catch (CorefireAliPayException $e){
			$msg = $e->errorMessage();
	...
	final private function ReplyNotify($needSign = true)
	{
		...
		$this->SetVersion(CorefireAliPayConfig::VERSION);
		
		CorefireAliPayApi::replyNotify($this->ToXml());
	}
}