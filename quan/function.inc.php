...
function quan_info($qid){//查看板块具体情况
	...
	$result["tvalue2"]=$value["tvalue2"];//头像
...
function show_img($u_id){//查看会员头像
	global $conn;	
	 $sql = "SELECT u_img FROM  user_member  where u_id='".$u_id."'   ";
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
	...
	if($num > $perpage) {
		...
		$to = $curr_page + $page - $offset - 1;
			...
			if($curr_page > 1) {
				...
				$multipage .= " <li><span class='disabled'>&lt;&lt;</span><span class='disabled'>&lt;</span></li>";
			}
			for($i = $from; $i <= $to; $i++) {
				if($i != $curr_page) {
					$multipage .= " <li><a href='{$mpurl}&".$pramname."=$i'>$i</a></li>";
				} else {
					$multipage .= " <li><a class='thisclass'>$i</a></li>";
				}
			}
			$maxpage = ceil($num/$perpage);
			if($curr_page < $maxpage) {
				$multipage .= " <li><a href='{$mpurl}&".$pramname."={$nextpage}'>下一页</a><a href='{$mpurl}&".$pramname."={$maxpage}'>最后一页</a></li>";
			} else {
				$multipage .= " <li><span class='disabled'>&gt;</span><span class='disabled'>&gt;&gt;</span></li>";
			}
	}
