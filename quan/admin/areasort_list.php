...
switch (get_param('action')){
		...
		case 'addit':			
			...
			if($query){
				message('成功增加资料','areasort_list.php');			
			}else{
			
				message('增加资料出错','areasort_list.php');		
			}
		...
		case 'delete':	
			...
			if( $res > 0 )
			{
				message('删除成功！','areasort_list.php');
			}else{
				message('删除失败!','areasort_list.php');
			}
		...
		case 'update':
			...
			if($query){
				message('成功修改资料','areasort_list.php');			
			}else{
			
				message('修改资料出错','areasort_list.php');		
			}
...
function checkdel(myid){
	...
	if(confirm("注意：\n 您真的要删除该信息吗？")==true){
		location.href="areasort_list.php?action=delete&id=" + myid;
		return true;
	}
	return false;
}
...
<div class="mainright" style="background-color:#F7F7F7;">
	<div class="maintitle" style="background-color:#F7F7F7"><strong>地区分类列表</strong></div>
    <div class="maincon">
...
<?
adminListSort($conn,0,"parentid",1,"typename","ys_areatsort","sysid","areasort_list.php");
