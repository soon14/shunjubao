...
$sql ="SELECT * FROM follow_ticket where u_id='".$u_id."' order by end_time desc  limit ".$limit;		
$dingzhi_array = $objMySQLite->fetchAll($sql,'id');

foreach ($dingzhi_array as $key=>$value) {
	
	$show_member_info = show_member_info($objMySQLite,$dingzhi_array[$key]["follow_id"]);
