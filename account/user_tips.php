...
$sql ="SELECT * FROM user_addtips_log where u_id='".$u_id."' order by addtime desc  limit ".$limit;		
$data_array = $objMySQLite->fetchAll($sql,'id');

foreach ($data_array as $key=>$value) {
	
	$show_member_info = show_member_info($objMySQLite,$data_array[$key]["to_u_id"]);
...
if ($previousPage) {
    ...
    $previousUrl = jointUrl(ROOT_DOMAIN."/account/user_tips.php", $args);
}
$nextPage = false;
...
if ($nextPage) {
	...
	$args['start_time'] = $start_time;
	$args['end_time'] = $end_time;
    $nextUrl = jointUrl(ROOT_DOMAIN."/account/user_tips.php", $args);
}
