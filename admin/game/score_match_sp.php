	...
	foreach ($data as $matchInfo) {
		...
		foreach($had_arr as $key=>$value){
			...
			if($new_leida_had_array["a"]!=$had_arr[$key]["a"] || $new_leida_had_array["d"]!=$had_arr[$key]["d"] || $new_leida_had_array["h"]!=$had_arr[$key]["h"]){
				$had_unusual = "<span style='color:red'>(有变动)</span>旧:".$had_arr[$key]["date"]." ".$had_arr[$key]["time"]."<br/>新:".$new_leida_had_array["date"]." ".$new_leida_had_array["time"]."<br/>";
			}else{
				$had_unusual = "旧:".$had_arr[$key]["date"]." ".$had_arr[$key]["time"]."<br/>新:".$new_leida_had_array["date"]." ".$new_leida_had_array["time"]."<br/>";
			}
		...
		foreach($hhad_arr as $key=>$value){
			...
			if($new_leida_hhad_array["a"]!=$hhad_arr[$key]["a"] || $new_leida_hhad_array["d"]!=$hhad_arr[$key]["d"] || $new_leida_hhad_array["h"]!=$hhad_arr[$key]["h"]){
				$hhad_unusual = "<span style='color:red'>(有变动)</span>旧：".$hhad_arr[$key]["date"]." ".$hhad_arr[$key]["time"]."<br/>新:".$new_leida_hhad_array["date"]." ".$new_leida_hhad_array["time"]."<br/>";
			}else{
				$hhad_unusual = "旧:".$hhad_arr[$key]["date"]." ".$hhad_arr[$key]["time"]."<br/>新：".$new_leida_hhad_array["date"]." ".$new_leida_hhad_array["time"]."<br/>";
			}
...
$tpl->d('../admin/game/score_match_sp');


function new_leida_sp($sport,$pool,$m_id){
	$conn = @mysql_connect("localhost", "root","meijun820526") or die("Could not connect to database");
	@mysql_select_db("leida", $conn) or die ("Could not select database");
