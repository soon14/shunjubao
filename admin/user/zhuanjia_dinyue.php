...
$e_name = get_param('e_name');//


if($e_name){
	$where  .= " and e_name like'%".$e_name."%' ";
	$tpl -> assign('e_name',$e_name);
		...
		//分页
				...
				$totalRecord = $conn -> NumRows($conn -> Query("SELECT * FROM ".tname("booking")." where  1 $where "),0);
				echo $sql ="SELECT *  FROM ".tname("booking")."  where  1 $where ORDER BY sysid DESC LIMIT ".$start.",".$pageSize;			
...
$YOKA ['output'] = $tpl->r ('../admin/user/zhuanjia_dinyue');
