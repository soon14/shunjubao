...
$userGiftLogInfos = $objUserGiftLogFront->getsByCondtionWithField($start_time . ' 00:00:00', $end_time . ' 23:59:59', 'create_time', $condition, $limit, 'create_time desc');
...
if ($previousPage) {
	...
	$args['u_name'] = $u_name;
    $previousUrl = jointUrl(ROOT_DOMAIN."/admin/user/gift_log.php", $args);
}
...
if ($nextPage) {
	...
	$args['end_time'] = $end_time;
	$args['u_name'] = $u_name;
    $nextUrl = jointUrl(ROOT_DOMAIN."/admin/user/gift_log.php", $args);
}

$uids = array();
foreach ($userGiftLogInfos as $value) {
	$uids[] = $value['u_id'];
}
...
echo_exit($tpl->r('../admin/user/gift_log'));