...
function badDomainLocation() {
	...
	if (isset($_SERVER['HTTP_HOST']) && $_SERVER["HTTP_HOST"] == '202.85.221.248') {
		redirect('http://www.zhiying365.com' , 301);
	}
	//其他域名访问时
	$all_domain_preg_match = '/^http:\/\/([hsy|wzc]*\.){0,1}(wap|m|www|pma|cms|news|bbs)\.(zhiying|zhiying365|hsy|wzc)\.com$/i';
...
在活动期间，智赢会员通过客户端购彩，中奖者可获得消费返点具体安排如下：
中奖2000元以下，返4%；
中奖2001元—6000元，返5%；
中奖6001以上，返6%
...
function manul_prize($orderTicket) {
	...
	if (!$userTicketInfo) {
		return InternalResultTransfer::fail('用户订单未发现');
	}
