...
//log_result($uid."_".json_encode($_REQUEST), 'log_combination_submit_start', true);
//确认结果提示页


$sql = "select datetime from  user_ticket_all  where u_id = '".$uid."'  order by id desc limit 0,1 ";
$last_ticket = $objMySQLite->fetchOne($sql,'id');
$last_ticket_datetime = $last_ticket["datetime"];
if(!empty($last_ticket_datetime)){
	$between_times = time()-strtotime($last_ticket_datetime);
	if($between_times<3){//设定3秒内不允许重复投注
			log_result($between_times."---".$uid."-重复时间过短", 'repeat_ticket', true);
			$args = array('type' => 'fail', 'from' => $from, 'msg' => '投注失败');
...
if (in_array(Runtime::getUname(), $virtual_users)) {
	$is_virtual = true;
...
while ($multiple>0) {
	foreach($M as $k => $v){
		...
		$info['prize'] 		= '0.00';$info['odds'] = '';$info['return_id'] = '';$info['rerurn_code'] = '';$info['return_str'] = '';
...
// }
 ...
 $log_confirm["combination"] = $c;  
 $log_confirm["pool"] = $p;  
 $log_confirm["from"] = $from;  
 log_result(json_encode($log_confirm), 'log_combination_submit', true);
