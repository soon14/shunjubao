...
function quan_info($qid){//查看板块具体情况
	global $conn;	
	$sql ="SELECT * FROM ".tname("bascdata_value")." where bd_id=1 and tvalue='".$qid."' LIMIT 0,1";		
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
	$result["tvalue2"]=trim($value["tvalue2"]);//头像
	$result["tname"]=$value["tname"];//名称	
	
	$sql ="SELECT count(*) as nums FROM ".tname("post")." where qid='".$qid."' ";		
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
	$result["tnums"]=$value["nums"];//帖子数
	
	$sql ="SELECT count(*) as nums FROM ".tname("post_member")." where qid='".$qid."' ";		
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
	$result["qnums"]=$value["nums"];//进来圈子人数
...
function show_post_isman($u_id,$qid){//查看用户是否管理员
	global $conn;	
	$sql = "SELECT * FROM   ".tname("post_member")."  where u_id='".$u_id."' and qid='".$qid."'   ";
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
...
function show_nick($u_id){//查看会员姓名
	global $conn;	
	$sql = "SELECT u_nick FROM     ".tname("post_member")." where u_id='".$u_id."'   ";
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);

	if(empty($value["u_nick"])){
		$ename = $value["u_name"];
	}else{
		$ename = $value["u_nick"];
	}
...
function show_img($u_id){//查看会员头像
	global $conn;	
	$sql = "SELECT u_img FROM  zhiying.user_member  where u_id='".$u_id."'   ";
	$query = $conn->Query($sql);
	$value = $conn -> FetchArray($query);
...
function show_zj_id(){//查看所有推荐方案专家ID
	global $conn;	
	 $sql = "SELECT u_id FROM  ".tname("recommond")."  where ifuse=1  group by u_id  ";
...
function show_win_lv($e_id){
	global $conn;	
	$sql = "SELECT count(*) as nums,islottey FROM  ".tname("recommond")."  where  u_id='".$e_id."' and islottey in(1,2) group by  islottey ";
	$query = $conn->Query($sql);
	$r["zj"]=0;
	$r["wzj"]=0;
	while($value = $conn -> FetchArray($query)){
		if($value["islottey"]==1){//中奖
			$r["zj"] = 	$value["nums"];	
		}
		if($value["islottey"]==2){
			$r["wzj"] =	$value["nums"];
...
function show_dingyue_nums($bookid){
	global $conn;	
	 $sql = "SELECT count(*) as nums FROM  ".tname("booking")."  where  bookid='".$bookid."'";
	$query = $conn->Query($sql);
	$values=$conn->FetchArray($query);
	return $values["nums"];	
}
function show_recommond($sysid){
	global $conn;	
...
function multi($num, $perpage, $curr_page, $mpurl,$pramname="page") {
	...
	if($num > $perpage) {
		...
		$to = $curr_page + $page - $offset - 1;
			...
			//
			if($curr_page > 1) {
				$multipage .= " <li><a href='{$mpurl}&".$pramname."=1'>首页</a></li><li><a href='{$mpurl}&".$pramname."={$prepage}'>上一页</a> </li>";
			} else {
				$multipage .= " <li class='am-disabled'><a href='#'>&laquo;</a></li>";
			}
			for($i = $from; $i <= $to; $i++) {
				if($i != $curr_page) {
					$multipage .= " <li><a href='{$mpurl}&".$pramname."=$i'>$i</a></li>";
				} else {
					$multipage .= " <li class='am-active'><a>$i</a></li>";
				}
			}
			$maxpage = ceil($num/$perpage);
			if($curr_page < $maxpage) {
				$multipage .= " <li><a href='{$mpurl}&".$pramname."={$nextpage}'>下一页</a></li><li><a href='{$mpurl}&".$pramname."={$maxpage}'>最后一页</a></li>";
			} else {
				$multipage .= " <li class='am-disabled'><a href='#'>&raquo;</a></li>";
			}
	}
	return $multipage;
}//end
